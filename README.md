...
A clean, lightweight, safe, and instantly understandable Android interface for senior users.
...
- Fraud protection (Digital arrest via video calls) via guardian alerts:
  - Sends an SMS to the guardian regarding continuous activity if the user's phone is active for 3+ hours continuously
  - Repeats SMS every 1 hour if usage continues
  - It stops sending furthur SMS messages if there is a discontinuation in the phone's usage
